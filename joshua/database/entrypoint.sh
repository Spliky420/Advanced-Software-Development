...
DB_DIR="${DB_DIR:-/data}"
DB_FILE="${DB_FILE:-/data/portfolio.db}"
SQL_DIR="${SQL_DIR:-/sql}"
...
if [ -f "$DB_FILE" ]; then
    ...
    # succeeded. A crash part-way leaves no portfolio.db, so the next start
    # retries cleanly instead of inheriting a half-seeded database.
    TMP_FILE="$DB_DIR/.portfolio.db.building"
    rm -f "$TMP_FILE"
...
echo "[database] ready: $(sqlite3 "$DB_FILE" 'SELECT COUNT(*) FROM holdings;') holdings, $(sqlite3 "$DB_FILE" 'SELECT COUNT(*) FROM allocation_targets;') allocation targets, $(sqlite3 "$DB_FILE" 'SELECT COUNT(*) FROM insight_log;') insight log rows"
